            ...
            AuthenticationProperties properties, OAuthTokenResponse tokens)
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                throw new HttpRequestException(
                    $"An error occurred when retrieving Github user information ({response.StatusCode}). Please check if the authentication information is correct and the corresponding Github API is enabled.");
            }

            using (var payload = await JsonDocument.ParseAsync(await response.Content.ReadAsStreamAsync())) // JObject.Parse(await response.Content.ReadAsStringAsync());
            {
